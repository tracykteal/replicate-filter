...
cdhit_command = '%s/cd-hit-est -i %s/tmp/input_fasta_file.fa -o %s/tmp/cdhit_output_temp -c %s -n 8 -s %s -d 0' % (batch_replicates_config.cdhit_dir, dirname, dirname, cutoff, length)
...
prog2 = subprocess.Popen('./extract-clusters-html.py %s/tmp/cdhit_output_temp.clstr  %s %s/extracted_clusters %s text -i "%s"' % (dirname, filename, dirname, bp_input, filename), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
